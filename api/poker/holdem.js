          ...
          var Card = /** @class */ (function () {
            function Card(suit, value) {
              ...
              if (typeof suit == 'string') {
                s =
                  suit.length == 1
                    ? SHORT_NAMES[suit.toLowerCase()]
                    : exports.Suits[suit];
                if (!s)
                  throw new Error(
                    'Invalid suit! Supported suits are: CLUB,DIAMOND,HEART,SPADE. You can also use the first letter as well.'
                  );
              } else s = suit;
              this.suit = s;
              if (value < 2 || value > 14)
                throw new Error(
                  'Invalid card value, should be 2 to 14 where 11=J, 12=Q, 13=K and 14=A'
                );
              this.value = value;
            }
            return Card;
        ...
        function (require, module, exports) {
          ...
          var Deck = /** @class */ (function () {
            function Deck() {
              var suits = [
                Card_1.Suits.CLUB,
                Card_1.Suits.DIAMOND,
                Card_1.Suits.HEART,
                Card_1.Suits.SPADE,
              ];
              this.cards = Array.apply(null, new Array(52)).map(function (
                _,
                v
              ) {
                return new Card_1.Card(suits[Math.floor(v / 13)], (v % 13) + 2);
              });
            }
          ...
          var Game = /** @class */ (function () {
            ...
            Game.prototype.getState = function () {
              ...
              return {
                ...
                pot: this.round.reduce(function (a, c) {
                  return a + c.money;
                }, this.pot),
                players: this.players.map(function (p, i) {
                  ...
                  return {
                    ...
                    active: p.active,
                    currentDecision:
                      (_this.round.length && _this.round[i].decision) || '',
                    currentBet:
                      (_this.round.length && _this.round[i].money) || 0,
            ...
            Game.prototype.startRound = function () {
              ...
              for (var i = 0; i < this.players.length; i++) {
                if (this.players[i].money < 0) this.players[i].active = false;
                else {
                  activePlayers++;
                }
                this.round[i] = {
            ...
            Game.prototype.bet = function (index) {
              ...
              const balance = this.players[index].money;
              const smallBalance = balance < this.initialBet;
              const actual = smallBalance ? balance : this.initialBet;

              this.round[index].money = actual;
              this.round[index].decision = 'bet';
              this.players[index].money -= actual;
            ...
            Game.prototype.raise = function (index, money) {
              if (!this.round.length) throw new Error('Game round not started');
              // if (this.round[index].money > money)
              //   throw new Error('cannot reduce position');
              // inactivePlayer?
              if (
                this.round[index].decision &&
                this.round[index].decision != 'raise'
              )
                throw new Error('Please proceed to next round');

              const balance = this.players[index].money;
              const smallBalance = balance < money;
              const baseAmount = smallBalance ? balance : money;
              const plusInitial = smallBalance
                ? balance
                : money + this.initialBet;

              const actual = !this.table.length ? plusInitial : baseAmount;
            ...
            Game.prototype.call = function (index) {
              ...
              const balance = this.players[index].money;
              let max_money = this.round.slice(0).sort(function (a, b) {
                return b.money - a.money;
              })[0].money;
              const smallBalance = balance < max_money;
              if (max_money == 0) max_money = this.initialBet; //fall back to initial betting amount
              const actual = smallBalance ? balance : max_money;
              const priorBet = this.round[index].money;
              // console.log('priorBet', priorBet);
              // bugged
              this.round[index].money = actual;
              this.round[index].decision = 'call';
              this.players[index].money -= actual - priorBet;
            ...
            Game.prototype.canEndRound = function () {
              ...
              for (var i = 0; i < this.round.length; i++) {
                ...
                if (last_amount < 0) last_amount = money;
                else if (money != last_amount) return false;
              }
              return true;
            };
            /**
            ...
            Game.prototype.endRound = function () {
              if (!this.round.length)
                throw new Error('Betting round not started');
              if (this.__roundStates.length == 4)
                throw new Error('Betting is over, please invoke checkResult');
              var last_amount = -1;
              var pot = 0;
              for (var i = 0; i < this.round.length; i++) {
                if (this.round[i].decision == 'fold') continue;
                ...
                if (last_amount < 0) last_amount = money;
                  ...
                  throw new Error(
                    'Round is not over yet, please call or raise.'
                  );
                pot += money;
              }
              this.pot += pot;
              ...
              if (this.__roundStates.length < 4) {
                ...
                this.table.push.apply(
                  this.table,
                  this.deck.getCards(communityCardCountForThisRound)
                );
              }
            };
            /**
            ...
            Game.prototype.checkResult = function () {
              var result = this.__instance.compareHands(
                this.players.map(function (m) {
                  return m.hand;
                }),
                this.table
              );
              if (result.type == 'win') {
                ...
                else throw new Error('This error will never happen');
              } else {
                //draw
                //split the pot among non-folded players
                var splitCount = this.players.filter(function (f) {
                  return !f.folded;
                }).length;
                if (splitCount > 0) {
      ...
      4: [
        ...
        */
            ...
            Holdem.prototype.TestPair = function (hand, mainCards) {
              for (var key in this.test_summary.value)
                if (this.test_summary.value[key].length == 2) {
                  this.test_cache.pair = {
                    value: Number(key),
                    index: this.test_summary.value[key][0],
                  };
                  return true;
                }
              return false;
            ...
            Holdem.prototype.TestTwoPairs = function (hand, mainCards) {
              ...
              for (var key in this.test_summary.value) {
                if (this.test_summary.value[key].length == 2) {
                  var _a = this.test_summary.value[key],
                    a = _a[0],
                    b = _a[1];
                  pair_indices.push(a);
                  all_indices.push(a);
            ...
            Holdem.prototype.TestThreeOfAKind = function (hand, mainCards) {
              for (var key in this.test_summary.value) {
                if (this.test_summary.value[key].length == 3) {
                  ...
                  this.test_cache.three_of_a_kind.indices =
                    this.test_summary.value[key];
                  return true;
                }
              }
            };
            ...
            Holdem.prototype.TestStraight = function (hand, mainCards) {
              ...
              var result =
                ...
                (seq_count == 3 &&
                  seq_start == 0 &&
                  seq[seq.length - 1].value == 14);
              this.test_cache.straight.result = result;
              if (result) {
                this.test_cache.straight.start = hand.findIndex(function (f) {
                  return (
                    f.suit == seq[seq_start].suit &&
                    f.value == seq[seq_start].value
                  );
                });
              }
              return result;
            ...
            Holdem.prototype.TestFullHouse = function (hand, mainCards) {
              if (
                this.test_cache.three_of_a_kind.indices.length == 3 &&
                this.test_cache.pair.value >= 2
              ) {
                return (
                  this.test_cache.three_of_a_kind.value !==
                  this.test_cache.pair.value
                );
              }
            };
            ...
            Holdem.prototype.TestStraightFlush = function (hand, mainCards) {
              if (
                this.test_cache.straight.result &&
                this.test_cache.flush.result
              ) {
                if (
                  hand[this.test_cache.straight.start].suit ==
                  this.test_cache.flush.suit
                ) {
            ...
            Holdem.prototype.TestRoyalFlush = function (hand, mainCards) {
              return (
                this.test_cache.straight_flush &&
                hand[this.test_cache.straight.start].value == 10
              );
            };
            Holdem.prototype.computeHand = function (allcards, mainCards) {
              ...
              for (var i = 0; i < allcards.length; i++) {
                ...
                else this.test_summary.suit[suit].push(i);
                if (!this.test_summary.value[value])
                  this.test_summary.value[value] = [i];
                else this.test_summary.value[value].push(i);
              }
              var result = this.TestSeries.map(function (t, i) {
                return i == 0
                  ? t.fn.call(_this, allcards, mainCards)
                  : t.fn.call(_this, allcards, mainCards)
                  ? i + 14
                  : 0;
              })
            ...
            Holdem.prototype.compareHands = function (hands, community) {
              ...
              var ranks = hands
                .map(function (f, index) {
                  ...
                  return __assign({}, _this.computeHand(hand, f), {
                    index: index,
                    cache: _this.test_cache,
                    hand: hand,
                  });
                })
                .sort(exports.DESC);
              if (ranks[0].value > ranks[1].value) {
                ...
                var result = __assign(
                  {},
                  TieBreaker_1.TieBreaker[highest_rank_name_1](
                    ranks.filter(function (r) {
                      return r.name == highest_rank_name_1;
                    })
                  ),
                  {
          ...
          var BreakTieUsingHiCard = function (ranks) {
            ...
            for (var i = 0; i < ranks.length; i++) {
              ...
              if (max_card_value <= hicard) {
                ...
                else max_card_map[max_card_value]++;
              }
            }
            if (max_card_index >= 0) {
          ...
          exports.TieBreaker = {
            ...
            Pair: function (ranks) {
              ...
              for (var i = 0; i < ranks.length; i++) {
                if (max_pair_val <= ranks[i].cache.pair.value) {
                  ...
                  else max_pair_counter['max' + max_pair_val].push(i);
                }
              }
              if (max_pair_index >= 0) {
                if (max_pair_counter['max' + max_pair_val].length == 1) {
                  return {
                    type: 'win',
                    index: ranks[max_pair_index].index,
                    value: max_pair_val,
                  };
                }
                var candidates = max_pair_counter['max' + max_pair_val].map(
                  function (m) {
                    var pair_value = ranks[m].cache.pair.value;
                    return __assign({}, ranks[m], {
                      hand: ranks[m].hand.filter(function (f) {
                        return f.value != pair_value;
                      }),
                    });
                  }
                );
            ...
            'Two pairs': function (ranks) {
              for (var check_highest = 0; check_highest < 2; check_highest++) {
                ...
                var max_pair_counter = {};
                for (var i = 0; i < ranks.length; i++) {
                  var _a = ranks[i].cache.two_pairs.pair_indices,
                    pa = _a[0],
                    pb = _a[1];
              ...
              var _loop_1 = function (i) {
                ...
                ranks[i].cache.two_pairs.all_indices
                  .sort(function (a, b) {
                    return b - a;
                  })
                  .forEach(function (f) {
                    return hand.splice(f, 1);
                  });
                if (max_hand_value <= hand[0].value) {
                  ...
                  else max_hand_counter['max' + max_hand_value]++;
                }
              };
              for (var i = 0; i < ranks.length; i++) {
                _loop_1(i);
              ...
              if (max_hand_index >= 0) {
                if (max_hand_counter['max' + max_hand_value] == 1)
                  return {
                    type: 'win',
                    index: ranks[max_hand_index].index,
            ...
            'Full House': function (ranks) {
              for (var check_3k = 0; check_3k < 2; check_3k++) {
                ...
                for (var i = 0; i < ranks.length; i++) {
                  var val =
                    ranks[i].cache[check_3k == 0 ? 'three_of_a_kind' : 'pair']
                      .value;
                  if (max_3k_val <= val) {
                    max_3k_index = i;
                    if (!max_3k_counter[max_3k_val])
                      max_3k_counter[max_3k_val] = 1;
                    else max_3k_counter[max_3k_val]++;
                  }
                }
                if (max_3k_index >= 0) {
                  if (max_3k_counter[max_3k_val] == 1) {
                    return {
                      type: 'win',
                      index: ranks[max_3k_index].index,
                      value: max_3k_val,
                    };
                  }
                }
            ...
            'Four of a kind': function (ranks) {
              for (var check_4k = 0; check_4k < 2; check_4k++) {
                ...
                var _loop_2 = function (i) {
                  ...
                  if (check_4k == 1) {
                    try {
                      val = ranks[i].hand.filter(function (f) {
                        return f.value != val;
                      })[0].value;
                    } catch (e) {
                      return 'break';
                  ...
                  if (max_4k_val <= val) {
                    ...
                    else max_4k_counter[max_4k_val]++;
                  }
                };
                for (var i = 0; i < ranks.length; i++) {
                ...
                if (max_4k_index >= 0) {
                  if (max_4k_counter[max_4k_val] == 1)
                    return {
                      type: 'win',
                      index: ranks[max_4k_index].index,
                      value: max_4k_val,
                    };
                }
              }
